...
target `172.30.0.20`, ICMP Echo, до 15 секунд и малое фиксированное число frames.
...
2  eth.src=02:42:ac:1e:00:14 arp.opcode=2 arp.src.proto_ipv4=172.30.0.20
...
## Разобранный пример 2: warm без нового ARP
...
Observed neighbor row плюс отсутствие ARP в bounded window согласуются с cache
reuse. Само отсутствие ARP без neighbor snapshot было бы слабее: capture мог
стартовать поздно или использовать неправильный interface/filter.

## Preflight, expected и stop conditions

1. `pnpm network:lab status` должен показать чистое состояние.
2. `pnpm network:lab preflight` должен PASS; image заранее загружен командой
   `pnpm network:lab preload` при необходимости.
3. До `up` заполните Expected в `evidence/comparison.md`: отдельно предскажите
   cold и warm timelines. Не копируйте туда «observed».
...
2. Создайте cold/warm evidence bundle. Runner очищает только neighbor entry `.20`
   ...
   Повтор создаёт новый run directory и не перезаписывает raw pcap.
3. Укажите путь к run directory в `evidence/comparison.md`. Перенесите фактические
   neighbor-before/after и normalized event rows, не фиксируя как invariant
   timestamps, IP ID, ICMP identifier или checksums.
...
- Local: `network-evidence` проверяет complete comparison, обе фазы, raw refs,
  cleanup status и отсутствие TODO.
...
- [ ] Expected cold/warm записан до action start; preflight PASS.
...
- [ ] Comparison ссылается на observed rows и не сравнивает variable fields как
      фиксированные.
- [ ] Cleanup/post-check подтверждают отсутствие lab resources.
